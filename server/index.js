 ...
 */
 ...
 const listen = async () => {
	 ...
	 if (startingPort !== port) log(log.SERVER, `Port ${startingPort} not available, using ${port} instead`)
	 ...
	 server.on('error', () => {
		 ...
		 log(log.TIPS, `Enable autoport for ${mode} mode if you want server to automatically pick available port`)
	 })
 }
 ...
 process.on('SIGINT', function() {
	 log(log.PROCESS, `Recieved CTRL + C. Shuttind down server`, { newline: true })
	 close()
 });
